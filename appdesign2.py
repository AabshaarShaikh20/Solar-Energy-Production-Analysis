...
try:
    model = joblib.load('solarproject.pkl')  
    st.write("<h2 style='color: #2E1A47;'>Model loaded successfully.</h2>", unsafe_allow_html=True)
except Exception as e:
    model = None
...
st.markdown("<h1 style='text-align: center; color: #006400;'>Solar Power Generation Prediction</h1>", unsafe_allow_html=True)

st.write("<p style='text-align: center; color: #3A5F0B;'>Enter the environmental variables to predict the solar power generation:</p>", unsafe_allow_html=True)
...
with st.form("prediction_form"):
    ...
    if submit_button:
        ...
        if prediction is not None:
            st.markdown(f"<h3 style='text-align: center; color: #FF6347; font-weight: bold;'>Predicted Power Generated: {prediction:.2f} kW</h3>", unsafe_allow_html=True)
        else:
            st.error("Prediction could not be made. Please check the model and inputs.")

# Custom CSS for button and output styles
st.markdown("""
    <style>
    .stButton button {
        color: #27742C;  /* Text color inside the button */
        ...
        border-radius: 8px;
        width: 100%;
        border: 7px solid green;  /* Green border */
    }
    .stButton button:hover {
        background-color: green;  /* Highlight color on hover */
        color: black;  /* Text color on hover */
    }
    h3 {
        color: #004225;  /* Output text color  */
        font-weight: bold;
        text-align: center;
    }
    </style>
